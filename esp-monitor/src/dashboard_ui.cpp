...
const lv_color_t kBg = lv_color_hex(0x06090D);
const lv_color_t kPanel = lv_color_hex(0x10161D);
const lv_color_t kPanelSoft = lv_color_hex(0x17212B);
const lv_color_t kGrid = lv_color_hex(0x24313C);
const lv_color_t kText = lv_color_hex(0xEDF7FF);
const lv_color_t kMuted = lv_color_hex(0x7D8A96);
const lv_color_t kCyan = lv_color_hex(0x46D9F7);
const lv_color_t kCyanSoft = lv_color_hex(0x153844);
const lv_color_t kOrange = lv_color_hex(0xFF952F);
const lv_color_t kOrangeSoft = lv_color_hex(0x4B2B12);
...
void build_background(lv_obj_t* screen) {
  ...
  for (int y = 18; y < kScreenHeight; y += 28) {
...
void build_header(lv_obj_t* screen) {
  ...
  lv_obj_set_size(header, 224, 40);
  lv_obj_align(header, LV_ALIGN_TOP_MID, 0, 8);

  lv_obj_t* title = make_label(header, &lv_font_montserrat_14, kText, LV_TEXT_ALIGN_LEFT, "CYBER INDUSTRIAL");
  lv_obj_align(title, LV_ALIGN_TOP_LEFT, 0, 0);

  lv_obj_t* chip = lv_obj_create(header);
  lv_obj_remove_style_all(chip);
  lv_obj_set_size(chip, 74, 18);
  ...
  lv_obj_t* sub = make_label(header, LV_FONT_DEFAULT, kMuted, LV_TEXT_ALIGN_LEFT, "AIRM2M CORE");
  ...
  lv_obj_set_size(rail, 224, 2);
  lv_obj_align(rail, LV_ALIGN_TOP_MID, 0, 52);
  lv_obj_set_style_bg_color(rail, kCyan, 0);
...
void build_cpu_panel(lv_obj_t* screen) {
  ...
  lv_obj_set_size(panel, 144, 160);
  lv_obj_align(panel, LV_ALIGN_TOP_LEFT, 8, 62);
  set_panel_style(panel, kCyan);
  add_corner_brackets(panel, kCyan);

  lv_obj_t* title = make_label(panel, LV_FONT_DEFAULT, kMuted, LV_TEXT_ALIGN_LEFT, "PRIMARY LOAD");
  lv_obj_align(title, LV_ALIGN_TOP_LEFT, 0, 0);

  lv_obj_t* cpu = make_label(panel, LV_FONT_DEFAULT, kCyan, LV_TEXT_ALIGN_LEFT, "CPU");
  lv_obj_set_style_text_letter_space(cpu, 2, 0);
  lv_obj_align(cpu, LV_ALIGN_TOP_LEFT, 0, 16);

  g.cpu_value = make_label(panel, &lv_font_montserrat_14, kText, LV_TEXT_ALIGN_LEFT, "056%");
  lv_obj_set_style_text_letter_space(g.cpu_value, 3, 0);
  lv_obj_align(g.cpu_value, LV_ALIGN_TOP_LEFT, 0, 42);

  lv_obj_t* util = make_label(panel, LV_FONT_DEFAULT, kMuted, LV_TEXT_ALIGN_LEFT, "UTIL");
  lv_obj_align(util, LV_ALIGN_TOP_LEFT, 0, 68);

  g.cpu_bar = lv_bar_create(panel);
  lv_obj_set_size(g.cpu_bar, 122, 10);
  lv_obj_align(g.cpu_bar, LV_ALIGN_TOP_LEFT, 0, 92);
  ...
  lv_obj_align(g.cpu_temp, LV_ALIGN_TOP_LEFT, 0, 116);

  g.cpu_freq = make_label(panel, LV_FONT_DEFAULT, kCyan, LV_TEXT_ALIGN_LEFT, "CLK 5.0GHz");
  lv_obj_align(g.cpu_freq, LV_ALIGN_TOP_LEFT, 0, 136);
}
...
void build_metric_panel(
  ...
  lv_obj_t* panel = lv_obj_create(screen);
  lv_obj_remove_style_all(panel);
  lv_obj_set_size(panel, 80, 74);
  lv_obj_align(panel, LV_ALIGN_TOP_LEFT, x, y);
  set_panel_style(panel, accent, 10);
  ...
  *out_value = make_label(panel, &lv_font_montserrat_14, kText, LV_TEXT_ALIGN_LEFT, "32%");
  lv_obj_align(*out_value, LV_ALIGN_TOP_LEFT, 0, 18);

  *out_bar = lv_bar_create(panel);
  lv_obj_set_size(*out_bar, 58, 8);
  lv_obj_align(*out_bar, LV_ALIGN_TOP_LEFT, 0, 42);
  ...
  *out_footer = make_label(panel, LV_FONT_DEFAULT, kMuted, LV_TEXT_ALIGN_LEFT, "TEMP 61C");
  lv_obj_align(*out_footer, LV_ALIGN_TOP_LEFT, 0, 56);
}

void build_bottom_strip(lv_obj_t* screen) {
  lv_obj_t* strip = lv_obj_create(screen);
  lv_obj_remove_style_all(strip);
  lv_obj_set_size(strip, 224, 76);
  lv_obj_align(strip, LV_ALIGN_BOTTOM_MID, 0, -8);
  set_panel_style(strip, kOrange, 12);
  lv_obj_set_style_bg_color(strip, lv_color_mix(kPanel, kOrangeSoft, LV_OPA_20), 0);
  add_corner_brackets(strip, kOrange);

  lv_obj_t* title = make_label(strip, LV_FONT_DEFAULT, kMuted, LV_TEXT_ALIGN_LEFT, "TELEMETRY");
  lv_obj_align(title, LV_ALIGN_TOP_LEFT, 0, 0);

  g.net_up = make_label(strip, LV_FONT_DEFAULT, kText, LV_TEXT_ALIGN_LEFT, "UP 048");
  lv_obj_align(g.net_up, LV_ALIGN_TOP_LEFT, 0, 20);

  g.net_down = make_label(strip, LV_FONT_DEFAULT, kText, LV_TEXT_ALIGN_LEFT, "DN 106");
  lv_obj_align(g.net_down, LV_ALIGN_TOP_LEFT, 0, 38);

  g.status = make_label(strip, LV_FONT_DEFAULT, kOrange, LV_TEXT_ALIGN_RIGHT, "STATUS // BOOST");
  lv_obj_align(g.status, LV_ALIGN_TOP_RIGHT, 0, 20);

  g.freshness = make_label(strip, LV_FONT_DEFAULT, kCyan, LV_TEXT_ALIGN_LEFT, "FRESH 82%");
  lv_obj_align(g.freshness, LV_ALIGN_BOTTOM_LEFT, 0, 0);

  g.freshness_bar = lv_bar_create(strip);
  lv_obj_set_size(g.freshness_bar, 92, 6);
  lv_obj_align(g.freshness_bar, LV_ALIGN_BOTTOM_RIGHT, 0, -2);
...
void update_dashboard(lv_timer_t* timer) {
  ...
  lv_label_set_text(g.status, hot ? "STATUS // HEAT" : "STATUS // BOOST");
...
void dashboard_ui_create(lv_display_t* display) {
  ...
  build_metric_panel(screen, 152, 62, "GPU", kCyan, &g.gpu_value, &g.gpu_bar, &g.gpu_footer);
  build_metric_panel(screen, 152, 148, "MEM", kOrange, &g.mem_value, &g.mem_bar, &g.mem_footer);
  build_bottom_strip(screen);
